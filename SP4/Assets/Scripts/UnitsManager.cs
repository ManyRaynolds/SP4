...
//	protected List<UnitsManager> nearbyObjects;
	...
	//		foreach(UnitsManager nearbyObject in nearbyObjects) 
			{
	//			Resource resource = nearbyObject.GetComponent<Resource>();
	//			if(resource) continue;
	//			if(nearbyObject.GetPlayer() != player) enemyObjects.Add(nearbyObject);
			}
	//		UnitsManager closestObject = WorkManager.FindNearestWorldObjectInListToPosition(enemyObjects, currentPosition);
...
//			UnitsManager worldObject = hitObject.transform.parent.GetComponent<UnitsManager>();
...
//	}
	...
	private void BeginAttack(UnitsManager target) {
		this.target = target;
		if(TargetInRange()) {
			attacking = true;
			PerformAttack();
